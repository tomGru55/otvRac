...
async function createRecord(record){
    ...
    try {
        const data = await fetch("/datatable/", {
            method: 'POST',
            headers: {
...
async function updateRecord(record, id){
    try {
        const data = await fetch("/datatable/" + id, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function loadData() {
    ...
    try {
        const data = await fetchData("filtered/" + "?keyword="+keyword+"&selectOption="+selectOption);
...
async function fetchData(path) {
    const response = await fetch("/datatable/" + path);
    if (!response.ok ) {
